...
/* 'nestedFunction' can access 'interal' because it is inside the 'myFunction' object. A function inside of
another function can access a variable defined in the parent function. So myFunction is the parent and nestedFunction is 
a function defined within it, and so nestedFunction is allowed to access 'internal'. */
...
function lowerCaseNames() {
  return zooAnimals.map(({ animal_name }) => animal_name.toLowerCase());
}
...
function lowPopulationAnimals() {
  return zooAnimals.filter(({ population }) => population < 5);
}
...
function USApop(/*Your Code Here*/) {
  return zooAnimals.reduce((prev, curr) => {
